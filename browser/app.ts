...
const ADPtoGNApairMap = new Map<RegExp, string>([
...
function parseMessageData(buffer) {
  ...
  var list: string[] = tdecoder.decode(buffer).split(/\0\0/);
...
function bufferToDataURI(buff, type?:string) {
  type = type || 'image/png';
...
const SoundPlayer: any = {
  ...
  beforeCreate() {
    ...
    this.analyserNode = <AnalyserNode> this.actx.createAnalyser();
    this.analdata = new Uint8Array(this.analyserNode.fftSize);

    this.currentAudio = <AudioBufferSourceNode|null> null;
    this.id = 1;
    this.intervalId;
...
interface FileItem {
  ...
  index: number, // current entire image offset
  entireIndex: number,
  length: number, // sublist.length
  sublist: ImageItem[],
}
interface ImageItem {
  fileName: string, // parent file name
  index: number,
  buffer: ArrayBuffer, // PNG data
  gnadat: GNA.GNAImageData,
  unusedColors: number[],
}
// main file list
const imageMap: Map<string, FileItem> = new Map();
const entireFileList: FileItem[] = [];
const entireImageList: ImageItem[] = [];
const EmptyPalette = new Uint32Array(16);
let slideshowTimeoutId: number;
let slideshowList: string[] = [];
let bustshotList: string[] = [];
const VueApp: any = {
  components: {
  ...
  mounted() {
    ...
    document.addEventListener('keydown', (ev) => {
      ...
      switch( (ev.target as HTMLElement).constructor ) {
    ...
    document.onpaste = (ev) => {
      const datatrans = ev.clipboardData as DataTransfer;
      const items = (datatrans.items);
      const list: (File|null)[] = [];
  ...
  data() {
    return {
      ...
      entireImageList,
      entireFileList,
      soundMap: new Map as Map<string, ArrayBuffer>,
      endMessages: null,
      ...
      SPR: {}, // sprites
      Temp: {}, // temporary variables used on HTML
      Tmpr: {}, // temp reserved (won't clear on reset)
  ...
  methods: <any>{
    ...
    move(dir) {
      ...
      const item: FileItem = this.currentFile;//entireFileList[this.fileIndex] || {};
      ...
      for (let retry = 2; retry--;) {
        switch (dir) {
          ...
          case 'up':
            ...
            const item: FileItem = entireFileList[this.fileIndex];
    ...
    drawPal() {
      const palcanvas = this.$refs.palcanvas as HTMLCanvasElement;
      palcanvas.width = 256;
      palcanvas.height = 16;
      ...
      const ctx2 = palcanvas.getContext('2d') as CanvasRenderingContext2D;
      for (let i = 0; i < 16; i++) {
    ...
    getFile(file: Event | FileList | File | File[]) {
      if (this.disabled) {
      ...
      if( file instanceof Event ) {
        list = (<HTMLInputElement>file.target).files!;
      }
    ...
    async startToConvert(filelist) {
      ...
      for (const file of filelist) {
        ...
        // GNA images
        let sublist: ImageItem[] = [];
        let imgcachelist: ImageData[] = [];
        const genimgs = GNA.extract(buffer, fileName, true) as Generator<GNA.GNAImageData>;
        for (const gnadat of genimgs) {
          ...
          const pngbuffer = <ArrayBuffer> await convertToPNGasync(imgdata);
          if( !this.disabled )
            break;
          ...
          const item: ImageItem = { fileName, index:sublist.length, buffer: pngbuffer, gnadat, unusedColors };
          sublist.push(item);
        }
    ...
    addGimmickResources(fileName: string, imgdataCache) {
      // add sprites
      ...
      if( P ) {
        this.SPR[P] = {...this.imageMap.get(fileName).sublist.map(item => bufferToDataURI(item.buffer))};
