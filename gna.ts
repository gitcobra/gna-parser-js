...
export interface GNAImageData {
  data: Uint8Array, // plain pixel data (all pixels are 0-15)
  palette: Uint32Array | null, // 16 color palette (32 bit)
  recommendedPalKey?: PalKeys, // adds by extract function

  length: number, // parsed byte length

  width: number,
  height: number,

  gnaType: number, // -1 if not GNA file (plain data such as .P files)
  gnaSize: number, // data size that GNA header indicates, if not GNA it's -1
  offsetX: number, // -1 if doesn't have offset
  offsetY: number, // -1 if doesn't have offset
  transparent: number, // transparent palette number. -1 if it doesn't have a transparent color

  _commands: Object | null // debug purpose
}
...
export function parseGNA(gnadat: ArrayBuffer, offset: number = 0): GNAImageData {
  ...
  const gnaType: number = dv.getUint16(6, true);
  const offsetX: number = dv.getUint16(8, true);
  const offsetY: number = dv.getUint16(0x0A, true);
  const width: number = dv.getUint16(0x0C, true); // * 8
  const height: number = dv.getUint16(0x0E, true);

  const transparent: number = dv.getInt16(0x10, true);
  const dataSize: number = dv.getUint16(0x12, true) << 4;
 ...
 */
export enum PalKeys {
  class1, class2, bustshot, kiba, obake, kissa, stage, level, op1, op2, op3
}
const Palettes: Map<PalKeys, Uint32Array> = new Map([
  // class room
...
export function getPalette(key: PalKeys): Uint32Array {
  if( !Palettes.has(key) )
    throw new Error(`unexpected Palette key "${key}"  keylist: ${Palettes.keys()}`);

  return Palettes.get(key) as Uint32Array;
  /*
...
export function parsePalette(dv:DataView | Uint8Array, offset:number = 0): Uint32Array {
  const pal: Uint32Array = new Uint32Array(16);
 ...
 * @return {*}  {Object}
 */
export function parseBETA(pdat: ArrayBuffer, offset: number = 0): GNAImageData {
  ...
  const offsetX: number = dv.getUint16(4, true); // offset x
  const offsetY: number = dv.getUint16(6, true); // offset y

  const width: number = dv.getUint16(0x08, true); // * 8
  const height: number = dv.getUint16(0x0A, true);

  const hoge1: number = dv.getUint16(0x0C, true); // always 2?
  const hoge2: number = dv.getUint16(0x0E, true); // always FFFF?
  const transparent: number = dv.getInt16(0x10, true);
 ...
 * @return {*}
 */
export function parsePlain(pdat: ArrayBuffer, width: number = 8, height: number = 64, offset: number = 0): GNAImageData {
  const dv = new DataView(pdat, offset);
  const dat = new Uint8Array(pdat, offset);
...
export function* iteratePlainImages(buffer: ArrayBuffer, width = 14, height = 128, count?: number, offset: number = 0): Generator<GNAImageData> {
...
* @return {*}  {Object}
*/
export function* seekImages(buffer: ArrayBuffer, offset: number = 0): Generator<GNAImageData> {
  ...
  for (let i = 0; i < length; i++) {
    ...
    for (const name in Signs) {
      ...
      // found signature
      try {
        let result: GNAImageData|null = null;
        switch (name) {
          case 'gna':
 ...
 */
export function* extract(buff: ArrayBuffer | Buffer, fname: string = '', getAsRaw: boolean = false): Generator<ImageData|GNAImageData> {
  let palkey: PalKeys | undefined;
  let foundPalette;
...
export function convertToImageData(dat: GNAImageData, pal?:Uint32Array | PalKeys): ImageData {
  const pix = dat.data;
  ...
  if( typeof pal !== 'undefined' ) {
    ...
    let recpal: PalKeys = dat.recommendedPalKey!;
    if( recpal! >= 0 )
...
export function convertADPtoWAVE(buff: ArrayBuffer | Buffer) {
